    ...
    def test_keychain_double(self):
        ...
        for secret_name in ("default", "sensitive", "developer", None):
            ...
            for value in ({"x": 2}, 4):
                ...
                self.assertEqual(kc.get_secret("name", secret_name=secret_name), value)

            self.assertEqual(tuple(kc.list_secrets(secret_name=secret_name)), ("name",))

            kc.delete_secret("name", secret_name=secret_name)

            self.assertEqual(tuple(kc.list_secrets(secret_name=secret_name)), ())
