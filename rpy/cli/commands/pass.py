...
class Command(SimpleCommand):
    ...
    def default_secret(self, name, password):
        return "!%s" % force_text(
            base64.dumps(
                hmac.new(
                    key=force_bytes(password), msg=force_bytes(name), digestmod=hashlib.sha1
                ).digest()
            )
        )
    ...
    def handle(
        ...
        **opts,
        ...
        location = validate(
            default_location or "~/.passwords/", "PASS_DEFAULT_LOCATION", name="location"
        )
        password = validate(default_password, "PASS_DEFAULT_PASSWORD", name="password")
        hashkey = validate(
            default_hashkey or default_password, "PASS_DEFAULT_HASHKEY", name="default_hashkey"
        )
