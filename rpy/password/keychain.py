...
def _parse_settings(opts):
    ...
    for name, value in opts.items():
        if not isinstance(value, dict):
            ...
            yield name, data(
                location=os.path.expanduser(value["location"]), password=value["password"]
            )
...
class KeyChain(object):
    ...
    def loads(self, payload, password):
        return self.serializer.loads(self.cypher.loads(payload, password=password))

    def dumps(self, payload, password):
        return self.cypher.dumps(self.serializer.dumps(payload), password=password)

    def get_location(self, secret_name, *paths):
        return os.path.join(self.settings[secret_name or self.default_name].location, *paths)
