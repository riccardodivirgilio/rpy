...
from rpy.dataframe.symbolic import evaluate, Symbolic
...
def handle(value):
    f = evaluate(value, missing_function=CallableText, context={}, default_context={})
    return Formula(f)
...
def handle(value):
    return Formula(
        value and "TRUE" or "FALSE", extimated_size=value and 4 or 5, needs_parenthesis=False
    )
...
def handle(value):
    return Formula(
        "%s" % value, needs_parenthesis=False, cell_format=("int_format", 3)  # #,##0
    )

@to_formula.dispatch((float, Decimal))
def handle(value):
    return Formula(
        "%s" % value, needs_parenthesis=False, cell_format=("dec_format", 4)  # #,##0.00
    )
    ...
    def write(self, value, y, x):
        ...
        if isinstance(formula, Formula):
            ...
            return self.worksheet.write_formula(
                ...
                x,
                formula.formula,
                cell_format=formula.cell_format
                and self.get_format(*formula.cell_format)
                or None,
            )
        elif formula:
