...
from rpy.dataframe.symbolic import evaluate, Symbol
...
class Copyable:
    ...
    def copy(self, **opts):
        defaults = dict((attr, getattr(self, attr)) for attr in self.__slots__)
...
class AtomData(object):
    ...
    def resolve_symbolic_excel_formula(self, symbol):
        ...
        if symbol.__symbolname__ == "dataframe":
            return ExcelDataFrameSymbol(dataframe=self.dataframe, workbook_name=None)

        if symbol.__symbolname__ == "column":
            return ExcelDataFrameSymbol(
                dataframe=self.dataframe, workbook_name=None, col=symbol.__symbolname__
            )
    ...
    def resolve_symbolic_value(self, symbol):
        ...
        raise KeyError(
            "Symbol %s not found. Choices are: dataframe, column, %s"
            % (symbol, ", ".join(self.dataframe.keys()))
        )
    ...
    def __repr__(self):
        return "<%s %s>" % (self.__class__.__name__, self._computed)
...
class ColumnData(object):
    ...
    def __repr__(self):
        return "<%s %s>" % (self.__class__.__name__, self.header.value())
...
class DataFrame(Mapping):
    def __init__(self, data, headers, dataframes=None):
        ...
        self.headers_dict = {header.value(): header for header in self.headers_list}

        if not isinstance(data, (tuple, list, set, frozenset)):
            data = tuple(data)

        self.table = tuple(
            ColumnData(data=data, header=header, dataframe=self)
...
class DataFrames(Mapping):
    def __init__(self, **kwargs):
        self.kwargs = {
            key: to_data_frame(value, dataframes=self) for key, value in kwargs.items()
        }
...
class ExcelDataFrameSymbol(Copyable, Symbol):
    ...
    def __init__(self, dataframe=None, workbook_name=None, row=None, col=None):
        ...
        if not col and not row:
            ...
            self.__symbolname__ = "%s:%s" % (row, row)
        else:
            self.__symbolname__ = "%s%s" % (self.dataframe[col].header.excel_index, row)
