...
def UnicodeReader(f, **opts):
    return import_string_and_call(
        "unicodecsv.py%i.UnicodeReader" % (six.PY2 and 2 or 3), f, **fixargs(opts)
    )

def UnicodeWriter(f, **opts):
    return import_string_and_call(
        "unicodecsv.py%i.UnicodeWriter" % (six.PY2 and 2 or 3), f, **fixargs(opts)
    )
...
def get_csv_dicts(path, header_processor=None, data_processor=data, **kwargs):
    ...
    if header_processor:
        ...
        headers = next(lines)
    return (data_processor(zip(headers, line)) for line in lines)
